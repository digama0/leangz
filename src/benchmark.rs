...
fn main() {
  ...
  // Read cache_dir from manifest if not specified on command line
  let cache_dir = cache_dir.or_else(|| {
    // Try to extract cache_dir from manifest header comments
    let file = File::open(&manifest_file).ok()?;
    let reader = BufReader::new(file);
    for line in reader.lines().map_while(Result::ok) {
      if line.starts_with("# cache_dir: ") {
        let path = line.strip_prefix("# cache_dir: ")?;
  ...
  // Fall back to ~/.cache/mathlib if no cache_dir in manifest
...
OPTIONS:
    ...
    <CACHE_DIR>                  Override cache directory (default: from manifest, or ~/.cache/mathlib)

EXAMPLES:
    # Create a manifest with isolated cache (downloads ~2GB)
    leantar-benchmark --create-manifest v4.26.0-rc2

    # Find optimal thread count using the isolated cache
    leantar-benchmark --sweep

    # Benchmark with your system's mathlib cache instead
    leantar-benchmark --sweep ~/.cache/mathlib
...
CACHE DIRECTORIES:
    When you run --create-manifest, cache files are downloaded to an isolated
    directory (benchmarks/cache-<TAG>/) for reproducible benchmarking.

    To test with your actual ~/.cache/mathlib (which may have more files or
    different filesystem characteristics), pass it as the CACHE_DIR argument.
"#
  );
}
...
/// 2. Downloads the cache files to an isolated directory (benchmarks/cache-<tag>/)
/// 3. Lists all downloaded .ltar files
/// 4. Writes the manifest to benchmarks/mathlib-<tag>.manifest with cache_dir metadata
fn create_manifest_for_tag(tag: &str) {
  ...
  // Use an isolated cache directory within benchmarks/ for reproducible benchmarking
  let cache_dir = benchmarks_dir.join(format!("cache-{}", tag));
  ...
  // Clean up any existing temp clone directory
  let _ = std::fs::remove_dir_all(&temp_dir);

  // Create cache directory (don't delete existing - allow incremental downloads)
  ...
  writeln!(file, "#").unwrap();
  writeln!(file, "# cache_dir: {}", cache_dir.display()).unwrap();
  writeln!(file, "#").unwrap();
  ...
  // Clean up temp clone directory (cache files are kept in benchmarks/cache-<tag>/)
  eprintln!();
  eprintln!("Cleaning up temporary clone directory...");
  let _ = std::fs::remove_dir_all(&temp_dir);

  eprintln!();
  eprintln!("Cache files are in: {}", cache_dir.display());
  eprintln!();
  eprintln!("To run the benchmark:");
  eprintln!("  leantar-benchmark --sweep");
  eprintln!();
  eprintln!("To benchmark with your system cache instead:");
  eprintln!("  leantar-benchmark --sweep ~/.cache/mathlib");
  eprintln!();
  eprintln!("Done!");
}
